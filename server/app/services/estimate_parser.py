...
Учитываются только листы с метками в названии: Q9, G9 (латиница) и РС (кириллица).
...
Особенности формата:
  ...
  • Коды норм РСНБ («1310-0701-0207») имеют другой формат (4-4-4) и под
    ...
    распознаётся正常 — у него код в формате АГСК.
...
# Q9 и G9 — латиница, РС — кириллица (Р и С).
SHEET_MARKERS = ("Q9", "G9", "РС")
...
def is_estimate_sheet(name: str) -> bool:
    """True — лист участвует в разборе (есть метка Q9, G9 или РС)."""
    n = (name or "").upper()
...
def match_estimate(items: List[Dict], estimate_items: List[Dict],
    ...
    """
    Проставляет позициям спецификации сметную цену.
    ...
    items изменяются на месте: добавляются estimate_price, estimate_match
    (способ связи) и estimate_name (что нашли в смете).
